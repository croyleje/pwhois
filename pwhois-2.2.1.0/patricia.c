...
/* Functions for counting overall memory */
#include "tst_malloc.h"

#include "patricia.h"

#define Delete tst_free
 ...
 */
int
subst_inet_pton (int af, const char *src, void *dst)
{
    u_long result;  
...
my_inet_pton (int af, const char *src, void *dst)
{
	...
	memcpy (dst, xp, 4);
        ...
        return (subst_inet_pton (af, src, dst));
#endif /* HAVE_IPV6 */
    } else {
#ifndef NT
	errno = EAFNOSUPPORT;
...
New_Prefix2 (int family, void *dest, int bitlen, prefix_t *prefix)
{
    ...
    if (family == AF_INET6) {
        default_bitlen = 128;
	if (prefix == NULL) {
            prefix = tst_calloc(1, sizeof (prefix6_t));
	    dynamic_allocated++;
	}
    ...
    if (family == AF_INET) {
		if (prefix == NULL) {
#ifndef NT
            prefix = tst_calloc(1, sizeof (prefix4_t));
#else
			//for some reason, compiler is getting
			//prefix4_t size incorrect on NT
			prefix = tst_calloc(1, sizeof (prefix_t)); 
#endif /* NT */
...
#else
			if ((result = subst_inet_pton (AF_INET6, string, &sin6)) <= 0)
				return (NULL);
#endif /* NT */
			return (New_Prefix (AF_INET6, &sin6, bitlen));
		}
...
{
    patricia_tree_t *patricia = tst_calloc(1, sizeof *patricia);
...
patricia_lookup (patricia_tree_t *patricia, prefix_t *prefix)
{
    ...
    new_node = tst_calloc(1, sizeof *new_node);
...
#ifdef PATRICIA_DEBUG
    fprintf (stderr, "make_and_lookup: %s/%d\n", prefix_toa (prefix), prefix->bitlen);
#endif /* PATRICIA_DEBUG */
...
try_search_exact (patricia_tree_t *tree, char *string)
{
    ...
    if (!prefix)
        return (NULL);
#ifdef PATRICIA_DEBUG
    fprintf (stderr, "try_search_exact: %s/%d\n", prefix_toa (prefix), prefix->bitlen);
#endif /* PATRICIA_DEBUG */
    node = patricia_search_exact (tree, prefix);
#ifdef PATRICIA_DEBUG
    if (node == NULL) {
        fprintf (stderr, "try_search_exact: not found\n");
    }
    else {
        fprintf (stderr, "try_search_exact: %s/%d found\n", 
	        prefix_toa (node->prefix), node->prefix->bitlen);
    }
#endif /* PATRICIA_DEBUG */
    Deref_Prefix (prefix);
...
try_search_best (patricia_tree_t *tree, char *string)
{
    ...
    if (!prefix)
        return (NULL);
#ifdef PATRICIA_DEBUG
    fprintf (stderr, "try_search_best: %s/%d\n", prefix_toa (prefix), prefix->bitlen);
#endif /* PATRICIA_DEBUG */
    node = patricia_search_best (tree, prefix);
#ifdef PATRICIA_DEBUG
    if (node == NULL)
        fprintf (stderr, "try_search_best: not found\n");
    else
        fprintf (stderr, "try_search_best: %s/%d found\n", 
	        prefix_toa (node->prefix), node->prefix->bitlen);
#endif /* PATRICIA_DEBUG */
    Deref_Prefix (prefix);
    return (node);
